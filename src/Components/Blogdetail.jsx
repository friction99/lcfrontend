    ...
    <div className="min-h-screen bg-gray-500 py-8 px-4">
      <div className="max-w-3xl mx-auto bg-white shadow-lg rounded-lg p-8">
        {editMode ? (
          ...
          <div>
            <h2 className="text-4xl font-bold mb-4 text-gray-900">{blogData.title}</h2>
            <p className="text-gray-700 text-lg leading-relaxed">{blogData.content}</p>
            {(blogData.author === author_id || blogData.author === user_id) && (
              <>
                <button className="p-2 my-2 bg-blue-500 rounded mr-4" onClick={handleEdit}>
                  Edit
                </button>
                <button className="p-2 my-2 bg-blue-500 rounded" onClick={handleDelete}>Delete</button>
              </>
            )}
          </div>
        )}
